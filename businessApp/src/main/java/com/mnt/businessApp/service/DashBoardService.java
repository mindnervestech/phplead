...
public class DashBoardService {
	...
	private Map getLeadProgressBarVM(String sql, String description,
			String iconClass, String type, int total) {
	...
	private Map getList(String description, String iconClass, String type,
		...
		vm.setType(type);
		vm.setHref("escalated-leads({leadType : '" + description
				+ "', editId : 'All' })");
		if (actualValue != 0 && total != 0) {
			vm.setValue(((Double) (((double) actualValue / (double) total) * 100))
					.intValue());
		}
	...
	public List<Map> getDashboardProgressbar(Date start, Date end, String zone,
			String state, Long product, Long dealer, String brand) {
		AuthUser user = Utils.getLoggedInUser();
		String sql = "";
		String sqlDate = " and l.lastDispo1ModifiedDate >= '"
		...
		if (dealer != 0 || product != 0 || !zone.equals("0")
				|| !state.equals("0") || !brand.equalsIgnoreCase("0")) {
			if (!zone.equals("0") && !state.equals("0")) {
				proZone += " and l.zone = '" + zone + "' and ld.state = '"
						+ state + "'";
			} else if (!zone.equals("0")) {
				proZone += " and l.zone = '" + zone + "'";
			} else if (!state.equals("0")) {
				if (user.getEntityName().equals("ZSM")
						|| user.getEntityName().equals("Sellout Manager"))
					proZone += " and ld.state = '"
							+ state
							+ "' and l.zone = (Select user.zone from user where user.id = "
							+ user.getEntityId() + " )";
			}
			if (!brand.equalsIgnoreCase("0")) {
				proZone += " and ld.lms = '" + brand + "' ";
			}
			if (product != 0) {
				proZone += " and ld.product_id =" + product;
			} else {
				if (user.getEntityName().equals("Category Manager")
						|| user.getEntityName().equals("Sellout-Regional")
						|| user.getEntityName().equals("RSM")
						|| user.getEntityName().equals("TSR")
						|| user.getEntityName().equals("Sales Consultant")) {
					sql = "select product.id from product where product.id IN (SELECT user_product.products_id from user_product WHERE user_product.User_id = "
							+ user.getEntityId() + ") ";
			...
			if (dealer != 0) {
				...
				if (user.getEntityName().equals("RSM")
						|| user.getEntityName().equals("TSR")
						|| user.getEntityName().equals("Sales Consultant")
						|| user.getEntityName().equals("Sales Executive")) {
					proZone += " and ld.pinCode IN (Select uz.zipcodes_id from user_zipcode uz where uz.user_id = "
							+ user.getEntityId() + " ) ";
				}
			}
			if (user.getEntityName().equals("RSM")
					...
					|| user.getEntityName().equals("Sales Consultant")
					|| user.getEntityName().equals("Sales Executive")) {
				proZone += " and (l.user_id = "
						+ user.getEntityId()
						+ "  or ld.pinCode IN (Select uz.zipcodes_id from user_zipcode uz where uz.user_id = "
						+ user.getEntityId() + " ) ) ";
			}
				...
				|| user.getEntityName().equals("Sales Consultant")
				|| user.getEntityName().equals("Sales Executive")) {
			proZone = " and ld.product_id IN (SELECT user_product.products_id from user_product WHERE user_product.User_id = "
					+ user.getEntityId() + ") ";
			if (user.getEntityName().equals("RSM")
					|| user.getEntityName().equals("TSR")
					|| user.getEntityName().equals("Sales Consultant")
		...
		} else if (user.getEntityName().equals("ZSM")
				|| user.getEntityName().equals("Sellout Manager")) {
			proZone = " and ( l.zone = (Select user.zone from user where user.id = "
					+ user.getEntityId()
					+ " )"
					+ " or l.user_id = "
					+ user.getEntityId() + " ) ";
		...
		if (user.getEntityName().equals("Admin")) {
			...
			sql = "SELECT COUNT(*) FROM lead l, leadDetails ld  where ld.id = l.leadDetails_id and l.user_id is null  "
					+ proZone + sqlDate;
			list.add(getLeadProgressBarVM(sql, "Overview",
					"icon fa fa-thumbs-up", "success", total));
		}

		return list;
	}

	public Map getZoneSplineBetweenDates(Date start, Date end, String zone,
			String state, Long product, String brand) {
		List<SplineVM> splineVMs = new ArrayList<>();
		AuthUser user = Utils.getLoggedInUser();
		if (user.getEntityName().equals("ZSM")
				|| user.getEntityName().equals("Sellout Manager")) {
			List<String> zones = jt.queryForList(
					"select user.zone from user where user.id = "
							+ user.getEntityId(), String.class);
			if (zones.isEmpty()) {
				return null;
			} else {
				zone = zones.get(0);
		...
		splineVMs.add(getSplineVMforZone(start, end, zone, state, product,
				" and status = 'Won' ", "Won", "#01c6ad", brand));
		splineVMs.add(getSplineVMforZone(start, end, zone, state, product,
				" and status = 'Lost' ", "Lost", "#FF0000", brand));
		splineVMs.add(getSplineVMforZone(start, end, zone, state, product,
				" and status = 'Open' ", "Open", "#ffce54", brand));
		Map<String, List<SplineVM>> map = new HashMap<>();
		map.put("dataset", splineVMs);
		return map;
	}

	private SplineVM getSplineVMforZone(Date start, Date end, String zone,
			String state, Long product, String query, String cat, String color, String brand) {
		AuthUser user = Utils.getLoggedInUser();
		...
		if (product == 0) {
			if (user.getEntityName().equals("Category Manager")
					|| user.getEntityName().equals("Sellout-Regional")) {
				productSql = "SELECT user_product.products_id from user_product WHERE user_product.User_id = "
						+ user.getEntityId();
			} else if (user.getEntityName().equals("General Manager")
					|| user.getEntityName().equals("CEO")
					|| user.getEntityName().equals("Admin")
					|| user.getEntityName().equals("Sellout Manager")
					|| user.getEntityName().equals("ZSM")) {
		...
		if (!zone.equals("0") && !state.equals("0")) {
			...
			select = "SELECT COUNT(*) as count, ld.state as name ";
			gropBy = " GROUP BY ld.state ORDER BY ld.state asc";
			all = "Select DISTINCT(ld.state) as name from lead l, leaddetails ld where l.leadDetails_id = ld.id and l.zone = '"
					+ zone + "' ORDER BY name asc";
		}

		List<Map<String, Object>> rows = jt.queryForList(select
		...
		for (Map row : allProducts) {
			...
			if (i < rows.size()) {
				Map data = rows.get(i);
				if (data.get("name").toString().toLowerCase()
						.equals(row.get("name").toString().toLowerCase())) {
	...
	public Map getProductSplineBetweenDates(Date start, Date end, String zone,
			String state, Long product, String brand) {
		List<SplineVM> splineVMs = new ArrayList<>();
		splineVMs.add(getSplineVMforProduct(start, end, zone, state, product,
				" and status = 'Won' ", "Won", "#01c6ad", brand));
		splineVMs.add(getSplineVMforProduct(start, end, zone, state, product,
				" and status = 'Lost' ", "Lost", "#FF0000", brand));
		splineVMs.add(getSplineVMforProduct(start, end, zone, state, product,
	...
	private SplineVM getSplineVMforProduct(Date start, Date end, String zone,
			String state, Long product, String query, String cat, String color, String brand) {
		SplineVM vm = new SplineVM();
		AuthUser user = Utils.getLoggedInUser();
		String productSql = "";
		...
		List<Map<String, Object>> rows = jt
				.queryForList("SELECT COUNT(*) as count, p.id as id from lead as l, leaddetails as ld, product as p "
						+ " where l.lastDispo1ModifiedDate > '"
						+ new SimpleDateFormat("yyyy-MM-dd").format(start)
						+ "' "
						+ " and  l.lastDispo1ModifiedDate < '"
		...
		System.out
				.println("Product :: "
						+ "SELECT COUNT(*) as count, p.id as id from lead as l, leaddetails as ld, product as p "
						+ " where l.lastDispo1ModifiedDate > '"
						+ new SimpleDateFormat("yyyy-MM-dd").format(start)
						+ "' "
						+ " and  l.lastDispo1ModifiedDate < '"
						+ new SimpleDateFormat("yyyy-MM-dd")
								.format(getDate(end)) + "' "
						+ " and ld.product_id = p.id and ld.product_id IN ("
						+ productSql + ") and ld.id = l.leadDetails_id "
						+ zoneState + query
						+ " GROUP BY ld.product_id ORDER BY ld.product_id asc");

		List<Map<String, Object>> allProducts = jt
				.queryForList("Select * from product where id In ("
						+ productSql + ") ORDER BY id asc");
		List<ZoneVM> productList = new ArrayList<ZoneVM>();
		int i = 0;
		List<List> list = new ArrayList<>();
		for (Map row : allProducts) {
	...
	public Map getDealerSplineBetweenDates(Date start, Date end, String zone,
			String state, Long product, Long dealer, String brand) {
		List<SplineVM> splineVMs = new ArrayList<>();
		splineVMs.add(getSplineDataForDealer(start, end,
				" and status = 'Won' ", "Won", "#01c6ad", state, product,
				dealer, brand));
		splineVMs.add(getSplineDataForDealer(start, end,
				" and status = 'Lost' ", "Lost", "#FF0000", state, product,
				dealer, brand));
		splineVMs.add(getSplineDataForDealer(start, end,
				" and status = 'Open' ", "Open", "#ffce54", state, product,
				dealer, brand));
		Map<String, List<SplineVM>> map = new HashMap<>();
		map.put("dataset", splineVMs);
		return map;
	...
	private SplineVM getSplineDataForDealer(Date start, Date end, String query,
			String cat, String color, String state, Long product, Long dealer,
			String brand) {
		AuthUser user = Utils.getLoggedInUser();
		if (!brand.equals("0")) {
			 query += " and ld.lms = '"+brand+"' ";
		...
		if (product != 0 && dealer != 0 && !state.equals("0")) {
			query += " and ld.product_id = " + product + "  and l.user_id = "
					+ dealer;
			if (user.getEntityName().equals("ZSM")
					|| user.getEntityName().equals("Sellout Manager"))
				query += " and ld.state = '"
			...
			if (user.getEntityName().equals("RSM")
					...
					|| user.getEntityName().equals("Sales Consultant")
					|| user.getEntityName().equals("Sales Executive")) {
				query += "and ld.product_id IN ( select products_id  from user_product  where User_id = "
						+ user.getEntityId() + " )";
			}
			...
			if (user.getEntityName().equals("ZSM")
				...
				|| user.getEntityName().equals("Sales Consultant")
				|| user.getEntityName().equals("Sales Executive")) {
			query += " and ld.product_id IN (SELECT user_product.products_id from user_product WHERE user_product.User_id = "
					+ user.getEntityId() + ") ";
			if (user.getEntityName().equals("RSM")
					|| user.getEntityName().equals("TSR")
					|| user.getEntityName().equals("Sales Consultant")
				...
				|| user.getEntityName().equals("Sellout Manager")) {
			User user1 = (User) sessionFactory.getCurrentSession().get(
					User.class, user.getEntityId());
			query += " and ( l.zone = (Select user.zone from user where user.id = "
					+ user.getEntityId() + " )";
		...
		if (user.getEntityName().equals("RSM")
				...
				|| user.getEntityName().equals("Sales Consultant")
				|| user.getEntityName().equals("Sales Executive")) {
			query += " and ld.pinCode IN (Select uz.zipcodes_id from user_zipcode uz where uz.user_id = "
					+ user.getEntityId() + " ) ";
		}
		return getDealerSplineVM(start, end, cat, color, query);
	}

	private SplineVM getDealerSplineVM(Date startDate, Date endDate,
			String cat, String color, String query) {
		SplineVM vm = new SplineVM();
		...
		for (Date date = start.getTime(); start.before(end); start.add(
			...
			if (i < rows.size()) {
				if (format.format(date).compareTo(
						format.format((Date) rows.get(i).get("date"))) == 0) {
